...
class SampleLayer(lasagne.layers.MergeLayer):
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        ...
        if deterministic:
            z = mu.dimshuffle(0,'x','x',1)*T.ones((batch_size, self.eq_samples, self.iw_samples, num_latent))
        else:
...
# def gumbel_softmax_sample(logits, shape, temperature=1): 
#   """ Draw a sample from the Gumbel-Softmax distribution"""
#   y = logits + sample_gumbel(shape)
#   return T.nnet.softmax( y / temperature)

# def gumbel_softmax(logits, shape=None, temperature=1, hard=False):
...
#   if shape is None:
#     shape = logits.shape
#   y = gumbel_softmax_sample(logits, shape, temperature)
#   # if hard:
#   #   k = logits.shape[-1]
#   #   #y_hard = tf.cast(tf.one_hot(tf.argmax(y,1),k), y.dtype)
#   #   y_hard = T.cast(T.eq(y,T.max(y,axis=1,keep_dims=True)),y.dtype)
...
class ConcreteSampleLayer(lasagne.layers.Layer):
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        ...
        if deterministic:
            ...
            z = p.dimshuffle(0,'x','x',1)*T.ones((batch_size, self.eq_samples, self.iw_samples, num_latent))
