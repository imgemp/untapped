...
from datasets.load_cuprite import load_process_data
...
res_out='examples/results/cuprite/'+timeStamp().format("")
...
m = SSDGM(num_features,num_output,variational=True,model_dict=model_dict,
          ...
          num_epochs=5000,eval_freq=100,lr=3e-3,eq_samples=1,iw_samples=1,
          res_out=res_out)
...
make_plots(m,data,colors,names,sample_size=10,ux=ux,remove_mean=remove_mean,res_out=res_out,title=title)
