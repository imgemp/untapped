...
from untapped.M12 import dirichlet, L2, KL
from untapped.S2S_DGM import SSDGM
from untapped.utilities import timeStamp

from datasets.load_raman import load_process_data, get_endmembers
from plotting.plotting_raman import make_plots
...
alpha = 1e-3*np.ones(num_output+1)
...
m = SSDGM(num_features,num_output,variational=True,model_dict=model_dict,
          ...
          coeff_x=1e-2,coeff_y=0,coeff_x_dis=1,coeff_y_dis=0,coeff_x_prob=0,coeff_y_prob=0,
...
make_plots(m,data,colors,names,groundtruth=get_endmembers(),
           waves=waves,sample_size=4,res_out=res_out,title=title)
