...
m = SSDGM(num_features,num_output,model_dict=model_dict,variational=True,
          prior_x=prior_x,prior_y=prior_y,prior_z2=prior_z2,loss_x=L2,loss_y=KL,
          coeff_x=1,coeff_y=1e-2,coeff_x_dis=1,coeff_y_dis=1e3,coeff_x_prob=1,coeff_y_prob=1e-2,
          num_epochs=100,eval_freq=500,lr=3e-3,eq_samples=1,iw_samples=1,
          default_data={'X_':data['X_'],'_y':data['_y'],'z2':data['z2']},res_out=res_out)

# define hyperparameter grid (+ any arguments that should be passed to fit)
model_parameters = {'coeff_x':np.logspace(-4,-2,num=3), 'coeff_y':np.logspace(-4,-2,num=3)}
fit_parameters = {'verbose':False,'debug':False}

# perform gridsearch with cross-validation
model = GridSearchCV(m, model_parameters, cv=2, fit_params=fit_parameters, verbose=2)
