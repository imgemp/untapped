...
from untapped.parmesan.distributions import log_normal
...
sup_train_x = sup_train_x[:3000]
sup_train_y = sup_train_y[:3000]
sup_valid_x = sup_valid_x[:3000]
sup_valid_y = sup_valid_y[:3000]
...
prior_z2 = lambda z2: log_normal(z2,mean=0.,std=1.).mean(axis=3)
...
arch['flows'] = []  # [partial(LogisticFlowLayer,lo=-5.1,hi=5.1)]
arch['batch_norm'] = False
...
for trial in range(num_trials):
  ...
  m = SSDGM(num_features,num_output,variational=True,model_dict=model_dict,eq_samples=1,iw_samples=1,
            ...
            coeff_x=1,coeff_y=0,coeff_x_dis=10,coeff_y_dis=0,coeff_x_prob=0,coeff_y_prob=0,
            num_epochs=1000,eval_freq=500,lr=3e-4,
            batch_size_Xy_train=100,batch_size_X__train=1000,batch_size__y_train=10,
            batch_size_Xy_eval=100,batch_size_X__eval=1000,batch_size__y_eval=10,
            res_out=res_out)
  ...
  endmembers = make_plots(m,data,colors,names,sample_size=4,res_out=res_out+'preperm',title=title)
