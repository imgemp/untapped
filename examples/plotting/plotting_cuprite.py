...
def make_plots(m,data,colors,names,groundtruth=None,waves=None,sample_size=10,ux=0,
    ...
    y_corners = np.vstack((np.eye(m.num_output),np.zeros(m.num_output))).astype('float32')

    if simplex is None:
        print('\tbuilding simplex for _y...')
        simplex = []
        for point in product(*([np.linspace(0,1,10)]*(m.num_output+1))):
            if np.sum(point) == 1:
                simplex += [point]
    ...
    if waves is None:
        waves = np.arange(data['X_'].shape[1])
    ...
    if m.model_type in [1,2]:
        z2_train = m.getZ2(x=data['X_'],deterministic=True)
        z2_train_mean = z2_train.mean(axis=0)
        z2_gen_train = z2_train_mean*np.ones_like(z2_train).astype('float32')
        z2_gen_manifold = z2_train_mean*np.ones((simplex.shape[0],z2_train.shape[1])).astype('float32')
