...
def make_plots(m,data,colors,names,waves=None,sample_size=10,ux=0,remove_mean=False,log_x=False,ylim=None,res_out=''):
    ...
    if m.variational:
        for endmember, dist, color, name in zip(endmembers,endmembers_dists,colors,names):
            ...
            if force_ylim:
                ax.set_ylim(ylim)
            plt.savefig(res_out+'/endmembers_dist_'+name+'.png',additional_artists=[lgd],bbox_inches='tight')
            plt.close()
    ...
    for endmember, color, name in zip(endmembers,colors,names):
        ...
        if force_ylim:
            plt.gca().set_ylim(ylim)
        plt.savefig(res_out+'/endmembers_mean_'+name+'.png',additional_artists=[lgd],bbox_inches='tight')
        plt.close()
