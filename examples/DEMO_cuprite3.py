...
from untapped.parmesan.layers import LogisticFlowLayer, ConcreteSampleLayer

from untapped.M12 import L2, KL
from untapped.S2S_DGM import SSDGM
from untapped.utilities import timeStamp

from datasets.load_cuprite import load_process_data, get_endmembers
...
train_x = train_x[np.random.choice(train_x.shape[0],10000),:]
...
data['z2_valid'] = np.random.uniform(low=-5, high=5, size=(train_y.shape[0], num_latent_z2)).astype('float32')
...
res_out='examples/results/cuprite2/'+timeStamp().format("")
...
# construct data dictionary
data['X'] = sup_train_x
data['X_valid'] = sup_valid_x
data['y'] = sup_train_y
data['y_valid'] = sup_valid_y

make_plots(m,data,colors,names,groundtruth=get_endmembers(),ylim=(0,1),
